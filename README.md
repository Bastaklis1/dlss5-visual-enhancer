...
- DLSS 5 Neural Rendering requires the bundled NVIDIA DLSS Neural Rendering runtime and a driver/GPU combination that successfully exposes the required feature through the Neuroframe Engine.
- Frame Interpolation requires a GPU, driver, and NVIDIA DLSS Frame Generation runtime combination that reports Frame Generation support. Hardware-accelerated GPU scheduling (HAGS) should be enabled; if it is disabled, the app reports it diagnostically and lets the runtime determine whether Frame Generation can start.
