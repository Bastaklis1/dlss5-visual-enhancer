...
from .models import ComparisonItem, DiffMetrics, build_comparison_items_from_paths
...
def send_paths_to_compare(input_paths, output_paths):
    """Send-to-Comparison handler for tabs (Image) whose render pipeline doesn't
    hand back a ready-made ComparisonItem list — reads the tab's own current
    sources/output components directly instead."""
    return receive_items(build_comparison_items_from_paths(input_paths, output_paths))
...
def bind_comparison_events(compare_tab: CompareTab, tabs, image_tab=None, grid_tab=None) -> None:
    ...
    if image_tab is not None:
        image_tab.send_to_compare.click(
            send_paths_to_compare, inputs=[image_tab.sources, image_tab.output_files],
