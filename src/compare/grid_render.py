...
from ..core.runtime import NR_STYLES, UPSCALING_CHOICES
...
GRID_AXES: dict[str, GridAxis] = {
    # v9.0 note: "NR Preset" and "DLSS Model Preset" axes removed here --
    # confirmed both nr_preset and dlss_model_preset fields (and their
    # choice dicts in core/runtime.py) no longer exist on
    # ImageConversionOptions at all. Not a rename to chase; the feature
    # they controlled is gone from the app.
    #
    # Not added here, but now available as real fields on
    # ImageConversionOptions if the Grid tab's sweep scope should grow to
    # cover them (a feature decision, not part of this port): the new
    # masking/composition controls -- nr_color_strength, tone_preservation,
    # face_skin_protection, grain_preservation, mask_feather, nr_passes.
...
def render_grid(
    ...
    # v9.0 note: this baseline-settings tail must stay positionally in sync
    # with neural_rendering.image.ui.build_neural_controls()'s real return
    # order (verified against the v9.0 source directly, not assumed):
    # [nr_style, nr_intensity, nr_passes, local_tone_strength,
    #  local_structure_strength, skin_structure_strength, upscaling_factor,
    #  automatic_mask] + build_composition_sliders() (5 more: color, tone,
    #  face_skin, grain, feather). nr_preset/dlss_model_preset removed --
    #  both fields are gone from ImageConversionOptions entirely at v9.0.
    #  nr_mask (custom mask upload) and nr_gpu_mode aren't part of that
    #  control tuple and aren't exposed as Grid baseline settings -- left at
    #  their ImageConversionOptions defaults (no mask, GPU mode on).
    nr_style: str, nr_intensity: float, nr_passes: float,
    local_tone_strength: float, local_structure_strength: float, skin_structure_strength: float,
    upscaling_factor: float, automatic_mask: str,
    nr_color_strength: float, tone_preservation: float, face_skin_protection: float,
    grain_preservation: float, mask_feather: float,
    ...
    base_kwargs = dict(
        ...
        nr_style=nr_style, nr_intensity=nr_intensity, nr_passes=int(nr_passes),
        ...
        nr_color_strength=float(nr_color_strength), tone_preservation=float(tone_preservation),
        face_skin_protection=float(face_skin_protection), grain_preservation=float(grain_preservation),
